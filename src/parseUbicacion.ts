...
/**
 * Interpreta cadenas sin separadores: primer dígito 1–4, cuatro dígitos de estantería, letra(s) de posición.
 * Ej.: "10065C" → pañol 1, estantería "0065", posición "C".
 * Ej.: "4000SP" → pañol 4, estantería "0000", posición "SP".
 */
export function parseUbicacion(raw: unknown): ParsedUbicacion | null {
  if (raw == null) return null;
  const s = String(raw).trim().replace(/\s+/g, "");
  if (!s) return null;
  const m = s.match(/^([1-4])(\d{4})([A-Za-z]+)$/);
  if (!m) return null;
  return {
    panol: Number(m[1]),
    estanteria: m[2],
